setwd('/Users/leichen/Desktop/Lab/Finmetseq_paper/1-Callset_QC/data/GS/highQ_anno/')
library(ggplot2)
library(tidyr)

count <- read.table('count_per_sample.gs.vcf4.txt', header = T)
...
pdf('plots/gs.sv_counts.per_sample.vcf4.ref2.pdf',width = 8, height = 5)
ggplot(df,aes(x=sample,y=var_count,fill=svtype))+
  geom_bar(stat="identity")+ggtitle("GS autosome SV counts by sample, ordered by sequencing date (VCF4)")+
...
ggplot(df,aes(x=sample,y=var_count,fill=sequencing_year))+
  ...
  scale_x_discrete(limits = sample_order)
ggplot(df,aes(x=sample,y=var_count,fill=svtype))+
  geom_bar(stat="identity")+ggtitle("GS autosome SV counts by sample (VCF4)")+
  theme(axis.text.x=element_blank(),axis.ticks.x=element_blank())+
...
pdf('plots/gs.sv_counts.per_sample.vcf4.refmode.pdf',width = 8, height = 5)
ggplot(df,aes(x=sample,y=var_count,fill=svtype))+
  geom_bar(stat="identity")+ggtitle("GS autosome SV counts by sample, ordered by sequencing date (VCF4)")+
  theme(axis.text.x=element_blank(),axis.ticks.x=element_blank())+
  scale_x_discrete(limits = sample_order)
